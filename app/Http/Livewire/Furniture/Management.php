...
class Management extends Component
{
    ...
    public function create($payload) {
        try {
            ...
            try {
                ...
                $this->emitTo('furniture.create-modal', 'show');
                $this->emit('flashSuccess', 'Furniture created');
            } catch (\exception $e) {
                $this->emit('flashError', 'Error trying to create furniture');
            }
        } catch (ValidationException $e) {
            foreach($e->errors() as $key=>$error) {
                $this->addError('create_' . $key, $error[0]);
            }
            $this->emit('createFurnitureErrorBag', $this->getErrorBag());
        }
    }
    ...
    public function update($payload) {
        try {
            ...
            try {
                ...
                $this->emitTo('furniture.edit-modal', 'show');
                $this->emit('flashSuccess', 'Furniture updated');
            } catch (\exception $e) {
                $this->emit('flashError', 'Error trying to update furniture');
            }
        } catch (ValidationException $e) {
            foreach($e->errors() as $key=>$error) {
                $this->addError('edit_' . $key, $error[0]);
            }
            $this->emit('editFurnitureErrorBag', $this->getErrorBag());
        }
    }
    ...
    public function delete($id) {
        try {
            ...
            try {
                ...
                $this->emit('flashSuccess', 'Furniture delete');
            } catch (\exception $e) {
                $this->emit('flashError', 'Error trying to delete furniture');
            }
        } catch (ValidationException $e) {
    ...
    {
        return view('livewire.furniture.management', [
