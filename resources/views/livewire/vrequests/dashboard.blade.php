            ...
            <table class="w-full whitespace-no-wrap">
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    ...
                    <tr class="text-gray-700 dark:text-gray-400 cursor-pointer hover:bg-gray-200
                        ...
                        @if(request()->route()->named('vrequests.index'))
                        onclick="window.location='{{ route('vrequests.show', $vrequest->id) }}';"
                        @else
                        onclick="window.location='{{ route('vrequests.show', $vrequest->id) }}';"
                        @endif>
        ...
        @if (!$vrequests->count())
            <tr>
                <p class="dark:text-gray-300 text-center mt-4 font-medium
                    text-gray-600">There are currently no Vacation Requests  .</p>
            </tr>
        @endif
    </div>
    {{ $vrequests->links() }}
